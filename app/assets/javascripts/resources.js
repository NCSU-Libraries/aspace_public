$(document).ready(function() {
  ...
  function skeletonTreeMode() {
    var skeletonTree = $('.resource-tree.skeleton-tree');
    return (skeletonTree.length > 0) ? true : false;
  }


  function resourceTreeSize() {
    var treeItems = $('.resource-tree .tree-item');
  ...
  function currentTab() {
    ...
    // check path_parts[1] for 'resources'
    // if (path_parts[1] == 'resources') {
    //   return (path_parts.length == 4) ? path_parts[3] : 'summary';
    // }

    // else if (path_parts[1].match(/^[a-zA-Z]+[\d_]+\d$/)) {
    //   return (path_parts.length == 3) ? path_parts[2] : 'summary';
    // } else {
    //   return 'summary';
    // }
  ...
  $.fn.extend({
        ...
        if ( (batch.length == batchSize) || (i == $(items).length) ) {
          ...
          $.get( getUrl, function( data ) {
            ...
            if (totalLoaded == treeSize) {
              ...
              $('.resource-tree .series-level').stickySeriesTitle();
            }
          });
          batch = [];
        }
    ...
    scrollToPosition: function() {
      ...
      // setTimeout(function() { window.scrollTo(0,targetPos); }, 1000);
  ...
  // console.log('targetArchivalObjectId = ' + targetArchivalObjectId);
  ...
  if (skeletonTreeMode()) {
    ...
    if (treeSize > 3000) {
      // console.log(treeSize + ' tree items');
