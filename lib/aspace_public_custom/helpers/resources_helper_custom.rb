module ResourcesHelperCustom
  ...
  ResourcesHelper.class_eval do


    def resource_overview
      if @presenter
        overview = '<div class="resource-overview row">'
        ...
        # overview << '<dt class="columns">Location</dt>'
        # overview << "<dd property=\"schema:provider arch:heldBy\" resource=\"http://www.lib.ncsu.edu/ld/onld/00000658\" class=\"columns\">#{location}</dd>"
        ...
        if @presenter.notes[:accessrestrict]
          ...
          overview << '<dt class="columns">Access to materials</dt>'
          overview << "<dd#{ rdfa_property_attribute(:accessrestrict) } class=\"columns\">#{ access_note }</dd>"
        end

        overview << '</dl>'

        if @presenter.has_digital_objects || @presenter.has_descendant_digital_objects
          overview << '<div class="columns">'
    ...
    def standard_use_note
      "<p>The nature of the NCSU Libraries' Special Collections means that copyright or other information
      about restrictions may be difficult or even impossible to determine despite reasonable efforts.
      The NCSU Libraries claims only physical ownership of most Special Collections materials.</p>
